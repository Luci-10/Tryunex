...
export async function sendOtpEmail(to: string, otp: string) {
  ...
  const from = `Wordrobe <${process.env.GMAIL_USER}>`;
  const text = `Your Wordrobe verification code is ${otp}. It expires in 10 minutes. If you didn't request this, you can ignore this email.`;
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
      <h2 style="color:#6d28d9;margin:0 0 12px;">Wordrobe</h2>
  ...
  `;
  await t.sendMail({ from, to, subject: "Your Wordrobe code", text, html });
}
